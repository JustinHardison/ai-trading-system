#!/usr/bin/env python3
"""
Train ML/RL from REAL MT5 CSV Export (100K bars)
...
def create_labels(df: pd.DataFrame, profit_target: float = 20.0, forward_bars: int = 5) -> np.ndarray:
    """Create AGGRESSIVE SCALPING labels - lower target for more frequent signals

    Changed from 40pts → 20pts for scalping frequency
    Target: 70-80% HOLD, 20-30% BUY/SELL (instead of 90% HOLD)
    """
    labels = []
...
def main():
    ...
    logger.info("="*70)
    logger.info("TRAINING FROM REAL MT5 EXPORTED CSV (100K BARS)")
    logger.info("="*70)

    # Load the CSV from command line args or default
    if len(sys.argv) >= 2:
        csv_file = sys.argv[1]
        symbol_name = sys.argv[2] if len(sys.argv) >= 3 else 'US30'
    else:
    ...
    if not Path(csv_file).exists():
        ...
        logger.info("3. Save as 'us30_historical_data.csv' in this folder")
        return

    # Auto-detect separator (tab or comma)
    df = pd.read_csv(csv_file, sep='\t')  # Try tab-separated first

    # Check if separator was correct (should have multiple columns)
    if len(df.columns) == 1:
        # Tab didn't work, reload with comma
        df = pd.read_csv(csv_file, sep=',')

    # Clean column names
    df.columns = [c.lower().strip() for c in df.columns]

    # Handle 'timestamp' column (from MT5 export) being renamed to 'time'
    ...
    if len(df) < 10000:
        logger.error(f"❌ Insufficient data: {len(df):,} bars (need 10K+)")
        return
    ...
    # 3. Create labels
    logger.info("\n[3/6] Creating scalping labels (20pt target, 5 bars forward)...")
    all_labels = create_labels(df, profit_target=20.0, forward_bars=5)
    ...
    ensemble_file = f"models/integrated_ensemble_{symbol_name.lower()}_{timestamp}.pkl"
    ensemble_data = {
    ...
    # Create symlink to latest for this symbol
    latest_link = Path(f'models/integrated_ensemble_{symbol_name.lower()}_latest.pkl')
    if latest_link.exists():
        latest_link.unlink()
    latest_link.symlink_to(Path(ensemble_file).name)

    logger.info(f"✓ Ensemble saved: {ensemble_file}")
    logger.info(f"✓ Symlink updated: models/integrated_ensemble_{symbol_name.lower()}_latest.pkl")

    # Save RL agent with symbol name
    rl_file = f"models/integrated_rl_{symbol_name.lower()}_{timestamp}.pkl"
    rl_agent.save(rl_file)

    rl_latest = Path(f'models/integrated_rl_{symbol_name.lower()}_latest.pkl')
    if rl_latest.exists():
        rl_latest.unlink()
    rl_latest.symlink_to(Path(rl_file).name)

    logger.info(f"✓ RL agent saved: {rl_file}")
    logger.info(f"✓ Symlink updated: models/integrated_rl_{symbol_name.lower()}_latest.pkl")

    # Print summary
    logger.info("\n" + "="*70)
    logger.info(f"✅ TRAINING COMPLETE - {symbol_name} MODEL!")
    logger.info("="*70)
    logger.info(f"Data source: MT5 CSV Export (REAL {symbol_name} M1 bars)")
