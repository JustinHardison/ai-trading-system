...
class HedgeFundPositionSizer:
    ...
    def calculate_position_size(
        ...
        # Increase risk for high quality trades
        quality_multiplier = (trade_quality + ml_confidence) / 2.0
        
        if quality_multiplier > 0.8:
            # Excellent trade: 0.5% risk
            risk_pct = self.max_risk_pct
        elif quality_multiplier > 0.7:
            # Very good trade: 0.4% risk
            risk_pct = self.max_risk_pct * 0.8
        elif quality_multiplier > 0.6:
            # Good trade: 0.3% risk
            risk_pct = self.max_risk_pct * 0.6
        else:
            # Decent trade: 0.25% risk
            risk_pct = self.base_risk_pct
