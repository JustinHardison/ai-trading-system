//+------------------------------------------------------------------+
//|                                  Export_Training_Data_CORRECT.mq5 |
//|                                                                    |
//| CORRECT SYMBOLS - Uses actual broker symbol names                 |
//+------------------------------------------------------------------+
...
// CORRECT SYMBOLS - ALL 8 symbols from your broker
string symbols[] = {
    "EURUSD.sim",    // Forex
    "GBPUSD.sim",
    "USDJPY.sim",
...
{
    Print("========================================");
    Print("EXPORTING TRAINING DATA - CORRECT SYMBOLS");
    Print("========================================");
    ...
    Print("");
    Print("🔄 Next: Tell me export is done!");
}
...
{
    // Clean symbol name for filename (remove .sim)
    string clean_symbol = symbol;
    StringReplace(clean_symbol, ".sim", "");
    StringReplace(clean_symbol, "Z25", "");
    StringReplace(clean_symbol, "F26", "");
    StringReplace(clean_symbol, "G26", "");
    
    // Create filename
    string filename = "training_data\\" + clean_symbol + "_" + tf_name + ".csv";
    ...
    if(copied <= 0)
    {
        Print("  ❌ ", tf_name, ": Failed (Error: ", GetLastError(), ")");
        return false;
    }
    ...
    {
        Print("  ❌ ", tf_name, ": Can't create file (Error: ", GetLastError(), ")");
        return false;
    }
