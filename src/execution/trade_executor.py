...
class TradeExecutor:
    ...
    def __init__(
        ...
        max_retries: int = 2,  # Reduced from 3 (faster failure detection)
        retry_delay: float = 0.3  # Reduced from 1.0s (faster retries)
    ):
        """
        Args:
        ...
        """
        # Use MT5 MQL5/Files (where EA actually writes files on Mac)
        import os
        mt5_common_files = Path.home() / "Library/Application Support/net.metaquotes.wine.metatrader5/drive_c/Program Files/MetaTrader 5/MQL5/Files"
        ...
        try:
            for attempt in range(self.max_retries):
                try:
                    ...
                    # Wait for response (timeout 5 seconds for faster operation)
                    start_time = time.time()
                    timeout = 5.0  # Reduced from 10s

                    while time.time() - start_time < timeout:
                        if self.response_file.exists():
                            # Minimal delay to ensure write is complete
                            time.sleep(0.05)  # Reduced from 0.1s

                            try:
                                # MT5 may write in different encodings - try multiple
                                response = None
                                for encoding in ['utf-8', 'utf-16-le', 'utf-16', 'cp1252', 'latin-1']:
                                    try:
                                        with open(self.response_file, 'r', encoding=encoding) as f:
                                ...
                                # Delete response file immediately after reading (v1.15+ EA expects this)
                                self.response_file.unlink()
