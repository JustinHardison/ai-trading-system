...
class IntelligentPositionManager:
    ...
    def analyze_position(self, context: EnhancedTradingContext) -> Dict:
        ...
        if context.distance_to_daily_limit < 1000 and current_profit_pct < 0:
            logger.info(f"⚠️ NEAR DAILY LIMIT - Closing loser")
            return {
                'action': 'CLOSE',
        ...
        # ═══════════════════════════════════════════════════════════
        # STEP 3: VOLUME & INSTITUTIONAL FLOW
        # ═══════════════════════════════════════════════════════════
        volume_divergence = context.volume_divergence if hasattr(context, 'volume_divergence') else 0.0
        ...
        # EXIT 2: MARKET QUALITY COLLAPSED (Most Important!)
        # If market no longer supports position, exit regardless of P&L
        if market_quality < 35:  # Market turned against us
            logger.info(f"")
            logger.info(f"❌ MARKET QUALITY COLLAPSED: {market_quality:.1f}/100")
            logger.info(f"   Position no longer supported by market structure")
            return {
                'action': 'CLOSE',
                'reason': f'Market quality collapsed to {market_quality:.1f}/100',
                'priority': 'HIGH',
                'confidence': 85.0
            }
        
        # EXIT 3: LOSING TRADE WITH WEAK MARKET
        # FIXED: Account for spread/commission - don't let tiny losses become big ones
        if current_profit_pct < -0.05:  # Any loss > -0.05% (spread + commission)
            # More aggressive exit if market quality weak
            if market_quality < 50:
                logger.info(f"")
                logger.info(f"❌ CUT LOSS: {current_profit_pct:.2f}% with weak market ({market_quality:.1f}/100)")
                return {
                    'action': 'CLOSE',
                    'reason': f'Loss {current_profit_pct:.2f}% with market quality {market_quality:.1f}/100',
                    'priority': 'HIGH',
                    'confidence': 80.0
                }
            
            # Cut bigger losses even with decent market
            if current_profit_pct < -0.15:  # -0.15% = ~$300 loss (was -0.25%)
                logger.info(f"")
                logger.info(f"❌ STOP LOSS: {current_profit_pct:.2f}%")
                return {
                    'action': 'CLOSE',
                    'reason': f'Stop loss hit: {current_profit_pct:.2f}%',
                    'priority': 'HIGH',
                    'confidence': 85.0
                }
        
        # EXIT 4: TRAILING STOP FROM PEAK
        # If we had a good profit and gave back too much
        if context.position_peak_profit_pct > 0.3:  # Had >0.3% profit
            if context.position_decline_from_peak_pct > 40:  # Gave back >40%
                logger.info(f"")
                logger.info(f"📉 TRAILING STOP: Peak {context.position_peak_profit_pct:.2f}%, declined {context.position_decline_from_peak_pct:.1f}%")
                return {
                    'action': 'CLOSE',
                    'reason': f'Trailing stop: Declined {context.position_decline_from_peak_pct:.1f}% from peak',
                    'priority': 'HIGH',
                    'confidence': 80.0
                }
        
        # EXIT 5: TAKE PROFIT - LOCK IN GAINS BEFORE COMMISSION EATS THEM
        # FIXED: Lower threshold - take smaller wins that stay positive after commission
        if current_profit_pct > 0.15:  # In profit > 0.15% (~$300 profit)
            # Take profit if market quality dropping
            if market_quality < 55:
                logger.info(f"")
                logger.info(f"💰 TAKE PROFIT: {current_profit_pct:.2f}% before reversal (quality: {market_quality:.1f})")
                return {
                    'action': 'CLOSE',
                    'reason': f'Take profit {current_profit_pct:.2f}% - market weakening',
                    'priority': 'HIGH',
                    'confidence': 75.0
                }
            
            # Take profit if exhaustion signals
            if exhaustion_score > 100:  # Multiple exhaustion signals (was 120)
                logger.info(f"")
                logger.info(f"💰 TAKE PROFIT: {current_profit_pct:.2f}% on exhaustion signals")
                return {
                    'action': 'CLOSE',
                    'reason': f'Take profit {current_profit_pct:.2f}% - exhaustion detected',
                    'priority': 'HIGH',
                    'confidence': 75.0
                }
        
        # EXIT 5B: TAKE BIGGER PROFIT IF STRONG
        # If profit > 0.3% and market still strong, let it run but watch closely
        if current_profit_pct > 0.3:  # Bigger profit
            # But exit if ANY weakness
            if market_quality < 60 or exhaustion_score > 80:
                logger.info(f"")
                logger.info(f"💰 TAKE PROFIT: {current_profit_pct:.2f}% - locking in gains")
                return {
                    'action': 'CLOSE',
                    'reason': f'Take profit {current_profit_pct:.2f}% - secure gains',
                    'priority': 'HIGH',
                    'confidence': 80.0
                }
        
        # EXIT 6: ML CONFIDENCE DROPPED SIGNIFICANTLY
        if context.ml_direction == original_direction:  # Still same direction
            if context.ml_confidence < 50:  # But weak confidence
                if current_profit_pct < 0.2:  # And not in strong profit
                    logger.info(f"")
                    logger.info(f"⚠️ EXIT: ML confidence dropped to {context.ml_confidence:.1f}%")
                    return {
                        'action': 'CLOSE',
                        'reason': f'ML confidence weak: {context.ml_confidence:.1f}%',
                        'priority': 'MEDIUM',
                        'confidence': 70.0
                    }
        ...
        # ═══════════════════════════════════════════════════════════
        # USE EV EXIT MANAGER - SOPHISTICATED AI EXIT LOGIC
        # This uses ALL 173 features to calculate Expected Value
        # ═══════════════════════════════════════════════════════════
        if self.ev_exit_manager is not None:
            symbol = getattr(context, 'symbol', 'UNKNOWN')
